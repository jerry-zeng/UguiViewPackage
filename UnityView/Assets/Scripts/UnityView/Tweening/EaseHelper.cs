...
namespace UnityView.Tweening 
{
	...
	public class EaseHelper 
    {
        ...
        private static float easeIn(float start, float end, float value){
            return start + end * pow3(value);
        }
        private static float easeOut(float start, float end, float value){
            return start + end *( pow3(value-1) + 1);
        }
        private static float easeInOut(float start, float end, float value){
            if( value*2f < 1f )
                return start + end * 0.5f * pow3(value*2f);
            else
                return start + end * 0.5f * (pow3(value*2f-2f) + 2f );
        }
