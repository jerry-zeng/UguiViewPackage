...
{
	[AddComponentMenu("uTools/Tween/Tween LayoutElement(uTools)")]
	
	public class uTweenLayoutElement : uTweenValue<float> {
		...
		public static uTweenLayoutElement Begin(GameObject go, float from, float to, float duration = 1f, float delay = 0f) {
			uTweenLayoutElement comp = Begin<uTweenLayoutElement>(go, duration);
            comp.value = from;
			comp.from = from;
			comp.to = to;
